...
def grid_detail(request, slug, template_name="grid/grid_detail.html"):
    ...
    * ``grid_packages`` - packages involved in the current grid
    ...
    grid_packages = gp.annotate(usage_count=Count('package__usage_count')).order_by('-usage_count', 'package')
    #grid_packages = gp.annotate(repo_watchers=Count('package__repo_watchers'))
